...
const PAPER = {
  ...
  subtitle: 'PDF rendered directly inside the React app',
  file: samplePaperUrl,
};

const GLOSSARY_KEY = `linkly:glossary:${PAPER.id}`;
const ANNOTATION_KEY = `linkly:annotations:${PAPER.id}`;
...
export default function PdfWorkbench() {
  ...
  const [glossaryEntries, setGlossaryEntries] = useState(() =>
    readSessionStorage(GLOSSARY_KEY, []),
  );
  const [annotations, setAnnotations] = useState(() =>
    readLocalStorage(ANNOTATION_KEY, []),
  );
  ...
  useEffect(() => {
    writeSessionStorage(GLOSSARY_KEY, glossaryEntries);
  }, [glossaryEntries]);

  useEffect(() => {
    writeLocalStorage(ANNOTATION_KEY, annotations);
  }, [annotations]);
